defmodule JustrunitWeb.Modules.Accounts.SignInLive do
  ...
  def render(assigns) do
    ...
    <div class="w-96 mx-auto mt-48 rounded-xl">
      ...
      <p class="text-center py-2">Click <.link navigate={~p"/sign-up"} class="text-blue-500 hover:underline text-lg">here</.link> if you don't have an account</p>
      <.form for={@form} phx-submit="save" action={~p"/sign_in"} class="space-y-8">
        ...
        <div>
          ...
          <p class="text-sm">Forgot your password? <.link navigate={~p"/reset_password"} class="text-blue-500 hover:underline">Reset it here</.link></p>
        </div>
        <.input field={@form[:remember_me]} type="checkbox" label="Keep me logged in" />
        <button type="submit" phx-disable-with="Logging in..." class="bg-blue-500 text-white px-4 py-2 rounded-lg font-semibold hover:bg-blue-600 transition w-full">Log in</button>
      </.form>
    </div>
    """
  end
